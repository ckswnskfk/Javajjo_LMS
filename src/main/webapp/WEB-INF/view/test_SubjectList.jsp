...
<table>
	...
	for(Subject_DTO dto: list){
		%>
		<tr>
			<td><a href="./test_Input.do?sujbectcode=<%=dto.getsubjectcode()%>&subjecttype=<%=dto.getsubjecttype()%>&examtype=<%=dto.getexamtype()%>&subjectname=<%=dto.getsubjectname()%>"><%=dto.getsubjectname() %></a></td>
